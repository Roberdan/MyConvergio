...
else echo "ERROR: Config file not found: $CONFIG_FILE"; exit 1; fi
...
if [[ "$CURRENT_HOST" == "omarchy" ]]; then
    echo -e "\033[0;31m[sync]\033[0m Run from Mac, not Linux. Usage: csync [push|pull|status]"
    exit 1
fi
...
cmd_status() {
    ...
    if git -C "$LOCAL_REPO" merge-base --is-ancestor "$rh" "$lh" 2>/dev/null; then
        local count; count=$(git -C "$LOCAL_REPO" rev-list --count "$rh".."$lh")
...
cmd_push() {
    ...
    if ! git -C "$LOCAL_REPO" merge-base --is-ancestor "$rh" "$lh" 2>/dev/null; then
        err "Cannot fast-forward. Run 'csync pull' first."; return 1
    fi
    ...
    if [[ "$lh" == "$new_rh" ]]; then
        ...
        err "Verification failed. Remote: ${new_rh:0:7}, expected: ${lh:0:7}"; return 1
    fi
}
...
cmd_pull() {
    ...
    local new_lh; new_lh=$(local_head)
    if [[ "$rh" == "$new_lh" ]]; then
        local count; count=$(git -C "$LOCAL_REPO" rev-list --count "$lh".."$new_lh")
        ok "Sync complete! Both at ${new_lh:0:7}"
        echo -e "\n  Pulled $count commit(s):"
        git -C "$LOCAL_REPO" log --oneline "$lh".."$new_lh" | sed 's/^/    /'
    else
        err "Verification failed. Local: ${new_lh:0:7}, expected: ${rh:0:7}"; return 1
    fi
}
...
case "${1:-status}" in
    ...
    *)  err "Unknown: $1. Use push|pull|status"; exit 1 ;;
esac
