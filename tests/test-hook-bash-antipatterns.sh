...
run_test() {
  ...
  # If we expect output, check for it
...
INPUT='{"tool_input":{"command":"find . -name \"*.txt\""}}'
run_test "Find command antipattern" "$INPUT" 0 "Glob"

# Test 2: Grep command should warn about Grep
echo "Test 2: Grep command warns about Grep tool"
INPUT='{"tool_input":{"command":"grep pattern file.txt"}}'
run_test "Grep command antipattern" "$INPUT" 0 "Grep"

# Test 3: Cat for reading should warn about Read
echo "Test 3: Cat command warns about Read tool"
INPUT='{"tool_input":{"command":"cat file.txt"}}'
run_test "Cat command antipattern" "$INPUT" 0 "Read"

# Test 4: Sed -i should warn about Edit
echo "Test 4: Sed -i warns about Edit tool"
INPUT='{"tool_input":{"command":"sed -i \"s/old/new/\" file.txt"}}'
run_test "Sed -i antipattern" "$INPUT" 0 "Edit"

# Test 5: Echo redirect should warn about Write
echo "Test 5: Echo redirect warns about Write tool"
INPUT='{"tool_input":{"command":"echo \"content\" > file.txt"}}'
run_test "Echo redirect antipattern" "$INPUT" 0 "Write"

# Test 6: Heredoc should warn about Write
echo "Test 6: Heredoc warns about Write tool"
INPUT='{"tool_input":{"command":"cat <<EOF > file.txt"}}'
run_test "Heredoc antipattern" "$INPUT" 0 "Write"

# Test 7: SQL != in double quotes should block (zsh issue)
echo "Test 7: SQL != in double quotes blocked for zsh safety"
INPUT='{"tool_input":{"command":"sqlite3 db.sqlite \"SELECT * WHERE status != '\''done'\''\""}}'
run_test "SQL != blocked" "$INPUT" 2 "BLOCKED"

# Test 8: SQL <> should pass
echo "Test 8: SQL <> operator passes"
INPUT='{"tool_input":{"command":"sqlite3 db.sqlite \"SELECT * WHERE status <> '\''done'\''\""}}'
run_test "SQL <> passes" "$INPUT" 0 ""

# Test 9: Pipe to grep should warn
echo "Test 9: Pipe to grep warns"
INPUT='{"tool_input":{"command":"ls -la | grep txt"}}'
run_test "Pipe to grep" "$INPUT" 0 "Grep"

# Test 10: Ripgrep (rg) should warn
echo "Test 10: Ripgrep warns about Grep tool"
INPUT='{"tool_input":{"command":"rg pattern ."}}'
run_test "Ripgrep antipattern" "$INPUT" 0 "Grep"

# Test 11: Head command should warn about Read
echo "Test 11: Head command warns about Read tool"
INPUT='{"tool_input":{"command":"head -n 10 file.txt"}}'
run_test "Head command antipattern" "$INPUT" 0 "Read"

# Test 12: Tail command should warn about Read
echo "Test 12: Tail command warns about Read tool"
INPUT='{"tool_input":{"command":"tail -n 10 file.txt"}}'
run_test "Tail command antipattern" "$INPUT" 0 "Read"

# Test 13: Awk should warn about Edit
echo "Test 13: Awk warns about Edit tool"
INPUT='{"tool_input":{"command":"awk '\''{print $1}'\'' file.txt"}}'
run_test "Awk antipattern" "$INPUT" 0 "Edit"

# Test 14: Normal git command should pass
echo "Test 14: Normal git command passes"
INPUT='{"tool_input":{"command":"git status"}}'
run_test "Git command passes" "$INPUT" 0 ""
...
# Test 16: Complex piped command with builtins should pass
echo "Test 16: Pipe to echo (builtin) passes"
INPUT='{"tool_input":{"command":"ls | echo test"}}'
run_test "Pipe to builtin" "$INPUT" 0 ""
