#!/usr/bin/env bash
# mesh-sync-all.sh v1.0.0
# Unified mesh sync: dotclaude config/DB + project repos + verification.
# Usage: mesh-sync-all.sh [--dry-run] [--peer NAME] [--phase PHASE] [--force]
#   Phases: all (default), config, repos, verify
#   --force: use git reset --hard instead of pull --ff-only (destructive)
set -euo pipefail
...
# Parse repos.conf → name|path|branch|gh_account
parse_repos() {
	[[ ! -f "$REPOS_CONF" ]] && return
	local name="" path="" branch="main" gh_account=""
	while IFS= read -r line || [[ -n "$line" ]]; do
		line="${line%%#*}"
		[[ -z "${line// /}" ]] && continue
		if [[ "$line" =~ ^\[(.+)\]$ ]]; then
			[[ -n "$name" && -n "$path" ]] && echo "$name|$path|$branch|$gh_account"
			name="${BASH_REMATCH[1]}"
			path=""
			branch="main"
			gh_account=""
...
# Target peers (excluding self, filtered by --peer)
get_targets() {
...
# SSH destination for peer → user@host
peer_dest() {
	local user route
...
# Phase 1: Push dotclaude config + dashboard DB
phase_config() {
	echo -e "${B}=== PHASE 1: Config + DB ===${N}"
	if git -C "$CLAUDE_HOME" status --porcelain 2>/dev/null | grep -q .; then
		echo -e "  ${Y}WARN${N}: uncommitted changes in ~/.claude (won't sync via git bundle)"
	fi
	if $DRY_RUN; then
		echo "  (dry-run) Would run: peer-sync.sh push"
	else
		"$SCRIPT_DIR/peer-sync.sh" push 2>&1 | sed 's/^/  /'
	fi
	echo ""
}

# Phase 2: Pull project repos on each peer
phase_repos() {
	echo -e "${B}=== PHASE 2: Repo Sync ===${N}"
	local repos
	repos="$(parse_repos)"
	if [[ -z "$repos" ]]; then
		echo "  No repos configured in $REPOS_CONF"
		echo ""
		return
	fi
	...
	while IFS= read -r peer <&3 || [[ -n "$peer" ]]; do
		...
		local peer_ok=true
		while IFS='|' read -r rname rpath rbranch raccount <&4 || [[ -n "$rname" ]]; do
			[[ -z "$rname" ]] && continue
			if $DRY_RUN; then
...
# Phase 3: Verify alignment across all peers
phase_verify() {
	echo -e "${B}=== VERIFICATION ===${N}"
	local repos repo_names=("dotclaude") repo_local_paths=("$CLAUDE_HOME") repo_remote_paths=("~/.claude")
	repos="$(parse_repos)"

	while IFS='|' read -r rn rp _ _ || [[ -n "$rn" ]]; do
		[[ -z "$rn" ]] && continue
		repo_names+=("$rn")
		repo_local_paths+=("${rp/#\~/$HOME}")
		repo_remote_paths+=("$rp")
	done <<<"$repos"
	...
	while IFS= read -r peer <&3 || [[ -n "$peer" ]]; do
		...
		# Single SSH call: one SHA per line
		local cmd=""
		for rp in "${repo_remote_paths[@]}"; do
			[[ -n "$cmd" ]] && cmd+="; "
...
*)
	echo "Unknown phase: $PHASE (use: all, config, repos, verify)" >&2
	exit 1
	;;
esac
