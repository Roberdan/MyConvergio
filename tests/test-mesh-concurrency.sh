...
setup() {
  export TEST_DB
  TEST_DB="$(mesh_test_create_db "" "CREATE TABLE IF NOT EXISTS waves(id INTEGER PRIMARY KEY, plan_id INTEGER, status TEXT DEFAULT 'pending');")"
  export TEST_TMP_DIR
  TEST_TMP_DIR="$(mesh_test_mock_ssh)"
...
teardown() {
  mesh_test_cleanup "${TEST_DB:-}" "${TEST_TMP_DIR:-}"
}
trap teardown EXIT
