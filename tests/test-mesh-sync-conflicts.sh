...
source "$REPO_ROOT/tests/lib/mesh-test-helpers.sh"
...
MOCK_BIN_DIR="$(mesh_test_mock_ssh)"
mkdir -p "$TEST_TEMP_DIR/local/data" "$TEST_TEMP_DIR/local/config" "$TEST_TEMP_DIR/remote"
...
make_db() {
  local db="$1"
  mesh_test_create_db "$db" "
    CREATE TABLE IF NOT EXISTS waves (id INTEGER PRIMARY KEY, wave_id TEXT, status TEXT, tasks_done INTEGER DEFAULT 0, tasks_total INTEGER DEFAULT 0, plan_id INTEGER);
  " >/dev/null
}
...
sqlite3 "$LOCAL_DB" "INSERT INTO plans(id,status,tasks_done,tasks_total) VALUES (1,'doing',0,1); INSERT INTO waves VALUES (1,'W1','doing',0,1,1);
  INSERT INTO tasks (id,task_id,plan_id,wave_id_fk,status,validated_at) VALUES (101,'T1',1,1,'in_progress','');"
sqlite3 "$REMOTE_DB" "INSERT INTO plans(id,status,tasks_done,tasks_total) VALUES (1,'doing',1,1); INSERT INTO waves VALUES (1,'W1','done',1,1,1);
...
make_db "$LOCAL_DB"; make_db "$REMOTE_DB"
write_peers_conf "$TEST_TEMP_DIR/local/config/peers.conf" "peer-a"
sqlite3 "$LOCAL_DB" "INSERT INTO plans(id,status,tasks_done,tasks_total) VALUES (1,'doing',1,1); INSERT INTO waves VALUES (1,'W1','done',1,1,1);
...
make_db "$LOCAL_DB"; make_db "$REMOTE_DB"
write_peers_conf "$TEST_TEMP_DIR/local/config/peers.conf" "peer-a"
sqlite3 "$LOCAL_DB" "INSERT INTO plans(id,status,tasks_done,tasks_total) VALUES (7,'doing',0,2); INSERT INTO waves VALUES (70,'W7','doing',0,2,7);
  INSERT INTO tasks (id,task_id,plan_id,wave_id_fk,status) VALUES (701,'T6a',7,70,'in_progress'),(702,'T6b',7,70,'pending');"
sqlite3 "$REMOTE_DB" "INSERT INTO plans(id,status,tasks_done,tasks_total) VALUES (7,'doing',1,2); INSERT INTO waves VALUES (70,'W7','doing',1,2,7);
