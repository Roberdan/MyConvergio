...
function _renderLastMission(m) {
  const p = m.plan || {},
    ...
    statusCls = p.status === 'done' ? 'badge badge-done' : 'badge badge-pending';
  let html = `<div class="mission-plan" onclick="openPlanSidebar(${p.id})"><div style="margin-bottom:6px"><span class="mission-id">#${p.id}</span><span class="mission-name">&nbsp;${esc(p.name || '')}</span>${statusDot(p.status)}${nodeLabel}${p.project_name ? `<span class="badge badge-project">${esc(p.project_name)}</span>` : ''}<span class="${statusCls}">${esc((p.status || '').replace(/_/g, ' '))}</span></div>${p.human_summary ? `<div class="mission-summary">${esc(p.human_summary)}</div>` : ''}<div class="mission-progress">${_progressRing(pct, 56, pg.color)}<div class="mission-progress-bars"><div class="mission-progress-label"><span>Overall status</span><span style="color:${pg.color}">${esc((p.status || 'done').toUpperCase())}</span></div><div class="mission-progress-track"><div class="mission-progress-fill" style="width:${pct}%;background:${pg.gradient}"></div></div><div style="display:flex;gap:8px;font-size:9px;color:var(--text-dim);margin-top:4px;flex-wrap:wrap"><span>${finishedAt ? `Finished ${_formatLastMissionTs(finishedAt)}` : ''}</span><span>${p.tasks_total ? `${p.tasks_done || 0}/${p.tasks_total || 0} tasks` : ''}</span></div></div></div>`;
  return html + '</div>';
}
