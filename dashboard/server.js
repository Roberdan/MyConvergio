...
const {
  routes: routesNotifications,
  handleSSE: handleNotificationSSE,
} = require("./server/routes-notifications");
  ...
  return new Promise((resolve, reject) => {
    ...
    function tryPort(port, attempts) {
      if (attempts >= maxAttempts) {
        reject(
          new Error(`No available ports found after ${maxAttempts} attempts`),
        );
        return;
      }
...
function matchRoute(method, url) {
  ...
  for (const [pattern, handler] of Object.entries(routes)) {
    ...
    const wildcardIndex = patternParts.findIndex((part) =>
      part.endsWith("(*)"),
    );
    ...
    if (wildcardIndex >= 0) {
      ...
      if (match) {
        ...
        const wildcardParam = patternParts[wildcardIndex]
          .slice(1)
          .replace("(*)", "");
...
const server = http.createServer((req, res) => {
  ...
  if (rawMatch) {
    const project = require("./server/db").query(
      `SELECT path FROM projects WHERE id = '${rawMatch[1]}'`,
    )[0];
    if (project?.path) {
...
findAvailablePort(BASE_PORT)
  .then((PORT) => {
    ...
    server.on("error", (err) => {
      ...
      if (err.code === "EADDRINUSE") {
        console.log(`Port ${PORT} became unavailable, finding a new port...`);
        findAvailablePort(PORT + 1)
          .then((newPort) => {
            ACTUAL_PORT = newPort;
            server.listen(newPort, "127.0.0.1", () => {
              console.log(
                `Dashboard API running at http://127.0.0.1:${newPort}`,
              );
            });
          })
          .catch((retryErr) => {
            console.error("Failed to find fallback port:", retryErr.message);
            process.exit(1);
          });
      } else {
        process.exit(1);
