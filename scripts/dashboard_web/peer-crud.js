...
function showPeerForm(mode = 'create', data = null) {
  ...
  const title = mode === 'create' ? 'Add Peer' : `Edit ${data?.peer_name || ''}`;
  ...
  overlay.addEventListener('click', e => { if (e.target === overlay) closePeerForm(); });
  document.getElementById('peer-form').addEventListener('submit', e => { e.preventDefault(); savePeer(mode); });
  document.getElementById('peer-ssh-btn').addEventListener('click', () => testSSH());
}
...
async function savePeer(mode) {
  ...
  const url = mode === 'create' ? '/api/peers' : `/api/peers/${encodeURIComponent(body.peer_name)}`;
...
async function testSSH() {
  ...
  try {
    const res = await fetch('/api/peers/ssh-check', {
      ...
      body: JSON.stringify({ ssh_alias: fd.get('ssh_alias'), tailscale_ip: fd.get('tailscale_ip'), user: fd.get('user') }),
    });
    const json = await res.json();
    st.textContent = json.ok ? `✅ ${json.latency_ms}ms` : `❌ ${json.error||'Failed'}`;
    st.className = json.ok ? 'ssh-ok' : 'ssh-fail';
  } catch (e) { st.textContent = '❌ Error'; st.className = 'ssh-fail'; }
}
...
function showDeleteDialog(peerName) {
  ...
  document.body.appendChild(overlay);
  overlay.addEventListener('click', e => { if (e.target === overlay) closePeerForm(); });
  overlay.querySelectorAll('input[name=delmode]').forEach(r => r.addEventListener('change', () => {
    document.getElementById('hard-confirm').style.display = r.value === 'hard' && r.checked ? '' : 'none';
  }));
  document.getElementById('do-delete-btn').addEventListener('click', () => deletePeer(peerName));
}

async function deletePeer(name) {
  const mode = document.querySelector('input[name=delmode]:checked')?.value || 'soft';
  if (mode === 'hard') {
    const v = document.getElementById('confirm-input')?.value;
    if (v !== 'ELIMINA') { alert('Type ELIMINA to confirm hard delete'); return; }
  }
  try {
    const res = await fetch(`/api/peers/${encodeURIComponent(name)}?mode=${mode}`, { method: 'DELETE' });
    const json = await res.json();
    if (json.error) { alert(json.error); return; }
    closePeerForm();
    if (typeof refreshMesh === 'function') refreshMesh();
...
async function showDiscoverOverlay() {
  ...
  try {
    ...
    if (!list.length) {
      overlay.querySelector('.peer-modal').innerHTML = '<h3>Discover Peers</h3><p>No new peers found.</p><button onclick="closePeerForm()">Close</button>';
      return;
    }
    ...
    list.forEach((p, i) => {
      html += `<label class="discover-item"><input type="checkbox" data-idx="${i}" checked>
        <b>${p.hostname}</b> — ${p.tailscale_ip} (${p.os})</label>`;
    });
    html += '</div><div class="peer-form-actions"><button onclick="closePeerForm()">Cancel</button>';
    html += '<button class="btn-primary" id="add-discovered-btn">Add Selected</button></div>';
    overlay.querySelector('.peer-modal').innerHTML = html;
    overlay.querySelector('#add-discovered-btn').addEventListener('click', async () => {
      ...
      for (const cb of checks) {
        ...
        await fetch('/api/peers', { method: 'POST', headers: {'Content-Type':'application/json'},
          body: JSON.stringify({ peer_name: p.hostname, ssh_alias: p.dns_name||p.tailscale_ip,
            user: 'roberdan', os: p.os, role: 'worker', tailscale_ip: p.tailscale_ip, dns_name: p.dns_name, status: 'active' })
        });
      }
      closePeerForm();
      if (typeof refreshMesh === 'function') refreshMesh();
    ...
    overlay.querySelector('.peer-modal').innerHTML = `<h3>Error</h3><p>${e.message}</p><button onclick="closePeerForm()">Close</button>`;
  }
}
