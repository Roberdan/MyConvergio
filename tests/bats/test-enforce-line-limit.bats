...
@test "enforce-line-limit: exits 0 when no file path in input" {
  run bash -c 'echo "{\"toolName\":\"edit\",\"toolArgs\":{}}" | bash "$1"' _ "$ENFORCER"
  [ "$status" -eq 0 ]
}

@test "enforce-line-limit: exits 0 for nonexistent file" {
  run bash -c 'echo "{\"toolName\":\"edit\",\"toolArgs\":{\"file_path\":\"/tmp/nonexistent_file_$$\"}}" | bash "$1"' _ "$ENFORCER"
  [ "$status" -eq 0 ]
}
...
@test "enforce-line-limit: allows file under 250 lines" {
  ...
  seq 1 100 > "$FILE"
  run bash -c 'echo "{\"toolName\":\"edit\",\"toolArgs\":{\"file_path\":\"$1\"}}" | bash "$2"' _ "$FILE" "$ENFORCER"
  [ "$status" -eq 0 ]
}
