...
# Version: 3.0.0 — Compact 1-line mini preview + theme support
...
# Compact node token for mini preview: "name(coord)" or "name(off)"
_mesh_mini_token() {
	local name="$1"
	local role=$(_mesh_get "${name}.role")
	local online=$(_mesh_get "${name}.online")
	...
	if [[ "$role" == "coordinator" || "$role" == "hybrid" ]]; then
		suffix="(coord)"
	elif [[ "$status" == "inactive" || "$online" != "1" ]]; then
		suffix="(off)"
	fi
	printf "${color}${name}${suffix}${NC}"
}

# Single compact line: "MESH: 2/3 online | m3max(coord) omarchy cloud-vm(off)"
_render_mesh_mini() {
	_mesh_collect_data
	local total=0
	...
	for n in $MESH_PEER_NAMES; do
		local tok
		tok=$(_mesh_mini_token "$n")
		tokens="${tokens:+$tokens }${tok}"
	done

	local online_color="${TH_SUCCESS:-$GREEN}"
	local count_str="${online_color}${MESH_ONLINE_COUNT}${NC}${GRAY}/${total}${NC}"
	printf "  ${TH_PRIMARY:-$CYAN}MESH${NC}${GRAY}:${NC} ${count_str} ${GRAY}online${NC}  ${GRAY}|${NC}  ${tokens}\n"
}
