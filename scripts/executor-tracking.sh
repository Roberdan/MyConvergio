...
executor_start() {
  ...
  [ -z "$project" ] || [ -z "$task_id" ] && { echo -e "${RED}Usage: executor_start <project> <task_id>${NC}"; return 1; }
...
executor_heartbeat_stop() {
  [ -n "$HEARTBEAT_PID" ] && { kill $HEARTBEAT_PID 2>/dev/null; export HEARTBEAT_PID=""; echo -e "${GREEN}Heartbeat stopped${NC}"; }
}
...
executor_log() {
  ...
  [ -z "$EXECUTOR_PROJECT" ] && { echo -e "${YELLOW}Not initialized. Run executor_start first.${NC}"; return 1; }
...
executor_complete() {
  ...
  [ "$status" = "failed" ] && { success="false"; status_value="failed"; }
  [ -z "$EXECUTOR_PROJECT" ] && { echo -e "${YELLOW}Not initialized${NC}"; return 1; }
