...
cmd_create() {
	...
	if [[ -d "$expanded_wt" ]]; then
		...
		git -C "$project_path" fetch origin main 2>/dev/null || true
...
cmd_merge() {
	...
	# 4. Push
	git -C "$wt_path" push -u origin "$branch" 2>&1 || true

	# 5. DRY_RUN: skip PR creation, CI, merge
	...
	remote_repo=$(git -C "$wt_path" remote get-url origin 2>/dev/null | sed 's|.*github.com[:/]||;s|\.git$||' || true)
	if ! command -v gh >/dev/null 2>&1; then
		log_warn "gh not installed — skipping PR creation"
		db_query "UPDATE waves SET status='done', completed_at=datetime('now') WHERE id=${wave_db_id};" 2>/dev/null || true
		cmd_cleanup "$plan_id" "$wave_db_id"
		return 0
	fi
	pr_url=$(gh pr create --repo "$remote_repo" --base main --head "$branch" \
