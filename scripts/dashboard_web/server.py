...
def api_history(): return _api_history()
def api_task_status_dist(): return _api_task_status_dist()
def api_tasks_blocked(): return _api_tasks_blocked()
def api_assignable_plans(): return _api_assignable_plans()
def api_notifications(): return _api_notifications()
def api_events(): return _api_events()
def api_coordinator_status(): return _api_coordinator_status()
def api_coordinator_toggle(): return _api_coordinator_toggle()
def api_plan_detail(plan_id: int): return _api_plan_detail(plan_id)
...
def api_mesh_sync_status() -> list[dict]:
    ...
    active_peers = [{"peer_name": s, "user": cp[s].get("user", ""), "host": cp[s].get("ssh_alias", s)} for s in cp.sections() if cp[s].get("status", "active") == "active"]
    hb_map = {r["peer_name"]: r["last_seen"] for r in query("SELECT peer_name, last_seen FROM peer_heartbeats")}
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as pool:
        results = list(pool.map(lambda p: _check_peer_sync(p["peer_name"], p["user"], p["host"]), active_peers))
    for entry in results:
        entry["last_heartbeat_age_sec"] = int(now - hb_map[entry["peer_name"]]) if hb_map.get(entry["peer_name"]) else -1
...
class Handler(MiddlewareMixin, SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path in ROUTES:
            self._json_response(ROUTES[path]()); return
        if path == "/api/mesh/action":
            self._json_response(handle_mesh_action(qs, _SAFE_NAME)); return
        if path == "/api/mesh/action/stream":
            handle_mesh_action_sse(self, qs, _SAFE_NAME); return
        if path == "/api/mesh/fullsync":
            handle_fullsync_sse(self, qs); return
        if path == "/api/terminal":
            self._json_response(handle_terminal(qs, _SAFE_NAME)); return
        if path == "/api/plan/move":
            self._json_response(handle_plan_move(qs)); return
        if path == "/api/plan/preflight":
            api_preflight_sse(self, qs); return
        if path == "/api/plan/delegate":
            handle_plan_delegate(self, qs, _SAFE_NAME); return
        if path == "/api/plan/start":
            handle_plan_start_sse(self, qs); return
        if path == "/api/mesh/pull-db":
            handle_pull_remote_db(self, qs); return
        if path.startswith("/api/plan/"):
            pid = path.split("/")[-1]
            self._json_response(api_plan_detail(int(pid)) if pid.isdigit() else {"error": "invalid plan id"}, 200 if pid.isdigit() else 400); return
        if path in ("", "/"):
