#!/usr/bin/env bash
set -euo pipefail
# gh-auto-token.sh — Copilot CLI preToolUse hook
# Auto-injects GH_TOKEN based on repo path mapping.
# Version: 1.0.0

CONFIG_FILE="$HOME/.claude/config/gh-accounts.json"
INPUT=$(cat)

# Fast path: if config missing, allow without env
...
try:
    ...
    cmd = d.get('toolArgs', {}).get('command') or d.get('tool_input', {}).get('command', '')
    print(cmd)
except:
    print('')
" 2>/dev/null || echo "")
...
# Resolve working directory from hook payload, fallback to PWD
...
echo "{\"permissionDecision\":\"allow\",\"env\":{\"GH_TOKEN\":\"$TOKEN\"}}"
