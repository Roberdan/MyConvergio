...
def api_preflight_sse(handler, qs: dict):
    ...
    if pc:
        ...
        if pc.get("tailscale_ip"):
            candidates.append(
                (
                    "tailscale_ip",
                    f"{pc.get('user', '') + '@' if pc.get('user', '') else ''}{pc['tailscale_ip']}",
                )
            )
    ...
    _check(
        "SSH reachable",
        True,
        f"{target} via {ssh_dest} ({next((l for l, d in candidates if d == ssh_dest), '?')}) ✓",
    )
    ...
    try:
        ...
        _check(
            "rsync",
            False,
            "rsync not found locally — install with: brew install rsync (mac) / apt install rsync (linux)",
        )
    if rsync_ok:
        try:
            r = _ssh_run(
            ...
            if "RSYNC_MISSING" in r.stdout:
                ...
                install_cmd = (
                    "sudo apt-get install -y rsync 2>/dev/null || sudo yum install -y rsync 2>/dev/null || sudo pacman -S --noconfirm rsync 2>/dev/null"
                    if target_os == "linux"
                ...
                _check(
                    ...
                    "was missing — installed on remote ✓",
                )
            else:
    ...
    hb = query_one("SELECT last_seen FROM peer_heartbeats WHERE peer_name=?", (target,))
    hb_ok, age = bool(
        hb and hb["last_seen"] and int(time.time() - hb["last_seen"]) < 300
    ), (int(time.time() - hb["last_seen"]) if hb and hb["last_seen"] else -1)
    if hb_ok:
    ...
    try:
        dry = (
            ...
            + (["--exclude-from", exclude_file] if os.path.isfile(exclude_file) else [])
            + [claude_home, f"{ssh_dest}:~/.claude/"]
        )
        r = subprocess.run(dry, capture_output=True, text=True, timeout=30)
        n_files = (
            int(
                re.search(
                    r"Number of regular files transferred:\s*(\d+)", r.stdout
                ).group(1)
            )
            if re.search(r"Number of regular files transferred:\s*(\d+)", r.stdout)
            else 0
        )
        if n_files == 0:
            ...
            _check(
                ...
                (
                    ...
                    if sr.returncode == 0
                    else f"rsync failed: {(sr.stderr.strip().split(chr(10))[-1][:60] if sr.stderr.strip() else f'exit {sr.returncode}')}"
                ),
            )
    ...
    try:
        free_gb = int(
            _ssh_run(
                ssh_dest,
                "python3 -c \"import shutil; u=shutil.disk_usage('.'); print(u.free//(1024**3))\" 2>/dev/null || python -c \"import shutil; u=shutil.disk_usage('.'); print(u.free//(1024**3))\" 2>/dev/null || echo -1",
                timeout=10,
            )
            .stdout.strip()
            .split("\n")[-1]
...
def handle_plan_start_sse(handler, qs: dict):
    ...
    if target == "local" or peer_host_match("m3max", target):
        ...
        try:
            ...
            proc.terminate()
            handler._sse_send("done", {"ok": False, "message": "Timeout (600s)"})
        except Exception as e:
            handler._sse_send("done", {"ok": False, "message": str(e)})
    else:
