# DEPRECATED: Improvements merged into lib/plan-db-crud.sh
# This file is no longer sourced. Kept for reference.
...
cmd_add_wave_fixed() {
	...
	while [[ $# -gt 0 ]]; do
		...
		--assignee)
			if [[ -z "${2}" ]]; then
				log_error "Missing value for --assignee"
				set -u
				exit 1
			fi
			assignee=$(sql_escape "$2")
			shift 2
			;;
		--planned-start)
			if [[ -z "${2}" ]]; then
				log_error "Missing value for --planned-start"
				set -u
				exit 1
			fi
			planned_start=$(sql_escape "$2")
			shift 2
			;;
		--planned-end)
			if [[ -z "${2}" ]]; then
				log_error "Missing value for --planned-end"
				set -u
				exit 1
			fi
			planned_end=$(sql_escape "$2")
			shift 2
			;;
		--estimated-hours)
			if [[ -z "${2}" ]]; then
				log_error "Missing value for --estimated-hours"
				set -u
				exit 1
			fi
			estimated_hours="$2"
		...
		--depends-on)
			if [[ -z "${2}" ]]; then
				log_error "Missing value for --depends-on"
				set -u
				exit 1
			fi
			depends_on=$(sql_escape "$2")
			shift 2
			;;
		*)
			assignee=$(sql_escape "$1")
			shift
			;;
		esac
	done
