...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --tasks-before) TASKS_BEFORE="$2"; shift 2 ;;
        --tasks-after) TASKS_AFTER="$2"; shift 2 ;;
        --diff) DIFF_SUMMARY="$2"; shift 2 ;;
        *) shift ;;
    esac
done
...
CURRENT_VERSION=$(sqlite3 "$DB_FILE" "SELECT COALESCE(MAX(version), 0) FROM plan_versions WHERE project_id='$PROJECT_ID' AND plan_name='$PLAN_NAME'")
...
INSERT INTO plan_versions (
    ...
    $([ -n "$GIT_HASH" ] && echo "'$GIT_HASH'" || echo "NULL"),
    '$TIMESTAMP'
);
EOF
...
VALUES ('$PROJECT_ID', '$PLAN_NAME', '$PLAN_FILE', 'active', ${TASKS_AFTER:-0}, '$TIMESTAMP')
...
WHERE project_id = '$PROJECT_ID' AND plan_name = '$PLAN_NAME';
EOF
fi
...
if [[ -d "${CLAUDE_HOME}/.git" && -n "$GIT_HASH" ]]; then
    ...
    if [[ -n $(git status --porcelain "plans/${PROJECT_ID}/" 2>/dev/null) ]]; then
        ...
        sqlite3 "$DB_FILE" "UPDATE plan_versions SET git_commit_hash='$NEW_GIT_HASH' WHERE project_id='$PROJECT_ID' AND plan_name='$PLAN_NAME' AND version=$NEW_VERSION"
    fi
fi
