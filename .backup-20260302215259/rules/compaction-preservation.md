...
| **Thor validation workflow**     | per-task Thor → `validate-task`, per-wave Thor → `validate-wave`, 9 gates, "NEVER skip Thor"  | Agents self-report success; Thor is the only independent check |
