...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h) echo "Usage: mesh-db-sync-tasks.sh [--peer NAME] [--plan ID] [--dry-run]"; exit 0 ;;
    *)         shift ;;
  esac
done
...
_pull_from_peer() {
  ...
  while IFS= read -r line; do
    [[ "$line" == "===WAVES===" ]] && { section="waves"; continue; }
    [[ "$line" == "===PLANS===" ]] && { section="plans"; continue; }
    [[ -z "$line" ]] && continue
...
_sync_task() {
  ...
  [[ "$local_status" == "$tstatus" ]] && {
    ...
    if [[ "${ttokens:-0}" -gt "${local_tokens:-0}" ]]; then
      $DRY_RUN && { $VERBOSE && warn "UPDATE tokens $tid: $local_tokens → $ttokens"; return 0; }
