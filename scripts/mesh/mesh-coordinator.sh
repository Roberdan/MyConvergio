...
_notify() {
  local sev="$1" title="$2" msg="$3"; shift 3
...
# Offline detection + auto-reassign
_check_offline_nodes() {
  ...
  while IFS='|' read -r peer_name last_seen; do
    ...
    if [[ $age -ge $REASSIGN_THRESHOLD && $count -ge $REASSIGN_GRACE ]]; then
      _reassign_plans "$peer_name" "$active_plans"
      rm -f "$counter_file"
    else
      _warn "Node $peer_name stale (${age}s, grace $count/$REASSIGN_GRACE)"
    fi
...
_reassign_plans() {
  local dead_peer="$1" plan_ids="$2"
  local best_peer
  best_peer=$(_db "SELECT peer_name FROM peer_heartbeats WHERE peer_name != '$dead_peer' AND (unixepoch() - last_seen) < $OFFLINE_THRESHOLD ORDER BY CAST(json_extract(load_json,'$.cpu') AS REAL) ASC LIMIT 1;" || echo "")
  [[ -z "$best_peer" ]] && best_peer="$(peers_self 2>/dev/null || hostname -s)"
  while IFS= read -r pid; do
    [[ -z "$pid" ]] && continue
    _db "UPDATE plans SET execution_host='$best_peer' WHERE id=$pid;"
    _err "REASSIGNED plan #$pid: $dead_peer → $best_peer (offline)"
    _notify critical "Node $dead_peer offline" "Plan #$pid reassigned to $best_peer" \
...
cmd_stop() {
  [[ ! -f "$PID_FILE" ]] && { _warn "Not running"; return 0; }
  local pid
...
-h|--help|help)
  ...
  ;;
"") _err "No command. Use: start|stop|status|run-once"; exit 1 ;;
